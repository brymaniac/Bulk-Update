...
switch  ($msgBoxInput) {
    'Yes' {
        ForEach ($User in $Users) {
            ...
            $title = Get-ADUser -Filter "displayname -eq '$($User.$Title)'"
            $department = Get-ADUser -Filter "displayname -eq '$($User.Department)'"
            if ($ADUser -eq $null) {
...
A               Write-Host "Update $ADUser to $manager, $title, $department" -ForeGroundColor Green
             "Command Succeeded"
             $main_form.Close()
            }
            }
